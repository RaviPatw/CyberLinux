#!/usr/bin/env bash
# ============================
# filesystem_kernel_harden_safe.sh
# CyberPatriot-style Filesystem & Kernel Hardening (Safe)
# ============================
...
LOGFILE="/var/log/fs_kernel_harden_safe_${STAMP}.log"
UNDO_FILE="/root/fs_kernel_harden_safe_undo_${STAMP}.sh"

SUDOERS="/etc/sudoers"
SYSCTL_CONF="/etc/sysctl.conf"
TMP_DIRS=("/tmp" "/var/tmp")
SECURE_DIRS=("/etc")  # Only harden /etc, not /bin or /usr/bin
...
#!/usr/bin/env bash
set -e
# Undo commands generated by filesystem_kernel_harden_safe.sh
EOF
chmod 700 "$UNDO_FILE"

# Logging
exec > >(tee -a "$LOGFILE") 2>&1
log "[*] Starting safe Filesystem & Kernel Hardening (STAMP=$STAMP)"
...
# -------------------------
# Secure directories (safe)
# -------------------------
log "[*] Securing sensitive directories (safe)"
...
# Apply kernel parameters safely
sysctl -p || log "[-] sysctl apply failed, continuing"
